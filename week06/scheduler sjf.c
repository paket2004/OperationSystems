...
ProcessData find_next_process() {
    ...
    for (int i = 0; i < data_size; i++) {
        if (data[i].burst > 0 && data[i].at <= total_time && data[i].burst < min_burst) {
    ...
    if (next_process_idx == -1) {
        // No process has arrived yet; increment total_time and try again
        printf("Scheduler: Runtime: %u seconds. No processes have arrived yet.\n", total_time);
        total_time++;
...
void schedule_handler(int signum) {
    ...
    // If there is a running process, decrement its remaining burst
    if (running_process != -1 && ps[running_process] > 0) {
